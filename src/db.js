...
/**
 * When the trail last grew. The only evidence a user ever gets that the
 * machine is alive — and, if it is hours stale while tracking is on, the only
 * battery-optimisation detector available without a native module.
 *
 * @returns {Promise<number|null>} unix seconds
 */
export async function lastFixAt() {
  const d = await db();
  const row = await d.getFirstAsync('SELECT MAX(recorded_at) AS at FROM fixes');
  return row?.at ?? null;
}
